...
We can conclude some things from the correlation map: 1. Member year and weeks_as_member have a high negative correlation, so they cannot both be used. I am choosing to use weeks_as_member as one of my predictors as it is a continuous variable. I will also be using duration instead of difficulty.
...
```
The offers differ in popularity; some offers get completed more than the other. There is variation by offers on the rate of completion. That is why, we will build a multilevel model where we will vary the intercept by the offers.
```{r, echo=FALSE}
...
This dataset is very imbalanced with only lll instances of offers being completed and www of those not being completed. This makes it harder for the models to learn the instances where the offer is completed. We will under sample from the majority class and over-sample from the minority class to fix this issue. 
```{r, echo=FALSE}

# Perform oversampling using ovun.sample
starbucks <- ovun.sample(completed ~ age + gender + income + reward + id+ difficulty + weeks_as_member + offer_type + channels, data = starbucks, method = "both", N = 12000, seed =1)$data
...
summary(null_model)
```
The null model predicts the majority class for every new observation
```{r}
#The normal model
...
summary(try1)
```
The partial pooling model uses the different offers as groups to capture the variation of offers completed in them.
```{r}
#partial_pooling1
...
```
We will try to vary the slope as well.
```{r}
#partial_pooling2
...
```
We will try to vary the slope in this model.
```{r}
#varying slope
...
We will now get the misclassification errors of all of the models based on the test dataset. The misclassificatin error is calculated as the sum of the incorrectly predicted observations over the total observations.
```{r}
model_list <- list(null_model, try1, cp, partial_pool_model)
```
...
predicted_probabilities <- predict(try1, newdata = starbucks_test, type = "response")
predicted_classes <- ifelse(predicted_probabilities > 0.5, 1, 0)
 actual_classes <- starbucks_test$completed
...
We will also get the confusion matrices.
...
# Generate five different random samples
random_samples <- replicate(num_samples, {
  starbucks %>%
    group_by(person) %>%
    sample_n(size = 1) %>%
    ungroup()
...
# Function to perform modeling and calculate misclassification errors
model_and_evaluate <- function(sample) {
   sample <- ovun.sample(completed ~ age + gender + income + reward + id + difficulty + weeks_as_member + offer_type + channels,
                        data = sample, method = "both", N = 12000, seed = 1)$data
  sample$completed <- as.numeric(as.character(sample$completed))
  # Create training and test sets
  index <- createDataPartition(sample$completed, p = 0.8, list = FALSE)
  sample_train <- sample[index, ]
  sample_test <- sample[-index, ]
 

  # Modelling
  null_model <- glm(completed ~ 1, data = sample_train, family = "binomial")
  try1 <- glm(completed ~ income + age + gender + gender:income + gender:age + weeks_as_member + difficulty + reward,
              data = sample_train, family = "binomial")
  partial_pool_model <- glmer(formula = completed ~ age + income + gender + weeks_as_member + (1 | id),
                              family = binomial, data = sample_train)
  partial_pool_model2 <- glmer(formula = completed ~ income + age + gender + gender:income + gender:age + weeks_as_member +
                                     offer_type + difficulty + (1 | id),
                               family = binomial, data = sample_train)
  partial_pooling3 <- glmer(formula = completed ~ income + age + gender + gender:income + gender:age + weeks_as_member +
                                (1 + weeks_as_member | id),
                            family = binomial, data = sample_train)
  cp <- glm(completed ~ income + age + gender + gender:income + gender:age + weeks_as_member + id,
            data = sample_train, family = "binomial")

  model_list <- list(null_model, try1, cp, partial_pool_model, partial_pool_model2, partial_pooling3)

  # Calculate misclassification errors
  errors <- calculate_misclassification_errors(model_list, sample_test)
...
# Apply the function to each random sample
results <- lapply(random_samples, model_and_evaluate)
...
# Split the data into training and test sets
  ...
  # Varying slope model
  partial_pooling3 <- glmer(formula = completed ~ income + age + gender + gender:income + gender:age + weeks_as_member + (1 + weeks_as_member | id),
                            family = binomial,
                            data = starbucks_train)
  summary(partial_pooling3)
  ...
  model_list <- list(null_model, try1, partial_pool_model, partial_pool_model2, partial_pooling3, cp)
