...
#The normal model
logistic_model <- glm(`offer completed`~ income, data= starbucks_train, family= "binomial")
summary(logistic_model)

logistic_model <- glm(`offer completed`~ gender+ age+ income+ weeks_as_member, data= starbucks_train, family= "binomial")
summary(logistic_model)
...
#no pooling
no_pool_model <- glm(formula = `offer completed` ~ age + income + gender + weeks_as_member + (1 | id),
                        family = binomial,
                        data = starbucks)
...
predicted_probabilities <- predict(your_model, newdata = your_data, type = "response")
predicted_classes <- ifelse(predicted_probabilities > 0.5, 1, 0)

# Assuming 'response_variable' is the actual binary response variable in your dataset
actual_classes <- your_data$response_variable
...
```{r}
naive_predictor <- rep(0, nrow(starbucks_test))

# Evaluate the naive predictor on the test set
naive_predictor <- factor(naive_predictor, levels = levels(starbucks_test$`offer completed`))
accuracy <- confusionMatrix(naive_predictor, starbucks_test$`offer completed`) %>% 
  extract("overall") %>% 
  extract("Accuracy")

f1_score <- confusionMatrix(naive_predictor, starbucks_test$`offer completed`) %>% 
  extract("byClass") %>% 
  extract("F1")
