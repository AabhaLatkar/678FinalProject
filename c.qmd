...
#no pooling
fit_mixed_model <- function(starbucks_train, formula) {
  # Group the data by ID
  grouped_data <- split(starbucks_train, starbucks_train$id)
  
  # Fit a mixed-effects model for each group
  models <- lapply(grouped_data, function(group) {
    lmer(formula, data = group)
  })
  
  return(models)
}
fit_mixed_model(starbucks_train, formula)
# Example usage:
# Assuming 'starbucks_data' is your dataset and 'completed' is the binary outcome variable
# You can modify the formula based on your specific predictors
formula <- completed ~ income + age + gender + (1 | id)
models_by_id <- fit_mixed_model(starbucks_train, formula)
id_counts <- table(starbucks_train$id)
...
```
```{r}
model_list <- list(null_model, try1)
```
