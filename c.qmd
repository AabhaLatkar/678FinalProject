...
data <- merge(profile, transcript, by = "person", all.x = FALSE)
data$new <- 1
data <- as.data.frame(extract_data(data))
data <- data() %>%
  pivot_wider(names_from = event, values_from = new)
starbucks <- merge(data, portfolio, by = "id", all.x = TRUE)
