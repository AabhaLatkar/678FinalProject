...
standardized_data <- scale(starbucks$income)
...
#no pooling
no_pool_model <- glm(formula = `offer completed` ~ age + income + gender + weeks_as_member + (1 | difficulty),
                        family = binomial,
                        data = starbucks)
...
```{r}
starbucsetes.factor(data$offer_completed)
formula <- as.formula("`offer completed` ~ age + income + gender + weeks_as_member + difficulty + reward")                       

# Create a data frame with the formula
data_formula <- model.frame(formula, starbucks_train)

# Perform undersampling using downSample
undersampled_data <- downSample(data_formula, y = "`offer completed`")

# Check the class distribution after undersampling
table(undersampled_data$`offer completed`)
