...
factor(starbucks$`offer completed`)
...
index <- createDataPartition(starbucks$`offer completed`, p = 0.8, list = FALSE)
...
```{r}
#The null model
null_model <- glm(`offer completed`~ 1, data= starbucks_train, family= "binomial")
summary(null_model)

#The normal model
logistic_model <- glm(`offer completed`~ income+age+gender+weeks_as_member+difficulty+reward+offer_type, data= starbucks_train, family= "binomial")
summary(logistic_model)

#The normal model 2
logistic_model2 <- glm(`offer completed`~ difficulty + offer_type +reward, data= starbucks_train, family= "binomial")
summary(logistic_model2)

#no pooling
starbucks_train$`offer completed` <- factor(starbucks_train$`offer completed`)

partial_pool_model <- glmer(formula = `offer completed`~ age + income + gender + weeks_as_member + (1 | id),
                        family = binomial,
                        data = starbucks_train)
summary(no_pool_model)
try1 <- glm(`offer completed` ~ income + age + gender + gender:income + gender:age + weeks_as_member+ difficulty+reward, data = starbucks_train, family = "binomial")
hist(starbucks$income)

```

```{r}

starbucks_train$`offer completed` <- as.factor(starbucks_train$`offer completed`)

# Perform oversampling using ovun.sample
balanced_starbucks <- ovun.sample(`offer completed` ~ age + gender + income + reward + difficulty + weeks_as_member + offer_type + channels, data = starbucks_train, method = "both", p = 0.5, N = 12000, seed = 1)$data

# Check the class distribution after oversampling
table(balanced_starbucks$`offer completed`)
...
predicted_probabilities <- predict(no_pool_model, newdata = starbucks_test, type = "response")
predicted_classes <- ifelse(predicted_probabilities > 0.5, 1, 0)

# Assuming 'response_variable' is the actual binary response variable in your dataset
actual_classes <- starbucks_test$`offer completed`
